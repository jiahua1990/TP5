...
class Order extends Controller
{
     ...
     * @throws \think\exception\DbException
     */
    public function express($order_id, $extra_id = 0)
    {   
        // 订单信息
        $order = OrderModel::getUserOrderDetail($order_id, $this->user['user_id']);

        if($extra_id > 0) {
            $extra_express = $order['extra_express'];
            foreach ($extra_express as $key => $val) {
                if($val['id'] == $extra_id) {
                    $model = $val['express'];
                    $express = $model->dynamic($model['express_name'], $model['express_code'], $val['express_no']);
                    if ($express === false) {
