...
class User extends UserModel
{
    ...
    private function register($open_id, $data, $referee_id = null)
    {
        ...
        try {
            ...
            if (!$user && $referee_id > 0) {
                RefereeModel::createRelation($model['user_id'], $referee_id, self::$wxapp_id);
            }
            $this->commit();
        } catch (\Exception $e) {
            $this->rollback();
    ...
    public function getMenus()
    {
        ...
        // 判断砍价功能是否开启
        if (!BargainSettingModel::isOpen()) {
            unset($menus['my_bargain']);
        }

        // 判断签到功能是否开启
        if (!SignInModel::isOpen()) {
            unset($menus['sign_in']);
        }
