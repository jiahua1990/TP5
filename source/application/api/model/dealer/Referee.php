...
class Referee extends RefereeModel
{
     ...
     */
    public static function createRelation($user_id, $referee_id, $spare_wxapp_id = '')
    {
        // 分销商基本设置
        ...
        // 新增关系记录
        $model = new self;
        $model->add($referee_id, $user_id, 1, $spare_wxapp_id);
        // # 记录二级推荐关系
        if ($setting['level'] >= 2) {
            // 二级分销商id
     ...
     * @throws \think\exception\DbException
     */
    private function add($dealer_id, $user_id, $level = 1, $spare_wxapp_id = '')
    {
        // 新增推荐关系
        $wxapp_id = self::$wxapp_id;
        if($wxapp_id == "") $wxapp_id = $spare_wxapp_id;
        $create_time = time();
