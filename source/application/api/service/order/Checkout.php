...
class Checkout
{
    ...
    private function checkDefaultAddress(){
        ...
        if ($intraRegion == false) {
            $this->user['address_default'] = false; 
        }
    }
    ...
    private function setOrderPoints()
    {
        ...
        // 计算商品的可抵扣金额
        $this->setOrderGoodsPoints();
        // 积分抵扣总数量
        $pointsNum = helper::getArrayColumnSum($this->goodsList, 'points_num');
        // 条件：商品的可抵扣总金额不能是0元
        // 条件：用户的积分余额必须大于订单所需积分
        if ($pointsNum < 1 || $this->user['points'] < $pointsNum) {
            return false;
        }
        // 积分抵扣总金额
        $this->orderData['points_money'] = helper::number2(helper::getArrayColumnSum($this->goodsList, 'points_money'));
        // 积分抵扣总数量
        $this->orderData['points_num'] = $pointsNum;
        // 允许积分抵扣
     ...
     * @return bool
     */
    private function setOrderGoodsPoints()
    {   
        // 统计订单商品总金额,(单位分)
        $goodTotalPrice = 0;
        foreach ($this->goodsList as $goods) {
            $goodTotalPrice += $goods['total_price'];
        }
        ...
        foreach ($this->goodsList as &$goods) {
            ...
            // 积分抵扣比例为0时不抵扣
            if($setting['discount']['discount_ratio'] == 0) continue;
            // 会员折扣商品不参与积分抵扣
            if ($this->user['grade_id'] > 0 && !empty($this->user['grade']) && !$this->user['grade']['is_delete'] && $this->user['grade']['status']) {
                if($goods['is_enable_grade']) continue;
            }
            // 积分抵扣比例
            $deductionRatio = helper::bcdiv($goods['points_discount_ratio'], 100);
            // 计算抵扣金额
            $pointsMoney = helper::bcmul($goods['total_price'], $deductionRatio);
            // 计算抵扣积分数量
            $goods['points_num'] = helper::bcdiv($pointsMoney, $setting['discount']['discount_ratio'], 0);
            // 记录抵扣的金额
            $goods['points_money'] = $goods['points_num'] > 0 ? helper::bcmul($goods['points_num'], $setting['discount']['discount_ratio']) : 0.00;

            // 积分最多抵扣到0.01元
            $goodTotalPrice -= $goods['points_money'];
            if($goodTotalPrice == 0){
                $goods['points_money'] = $goods['points_money'] - 0.01;
            }
        }
        return true;
    }
