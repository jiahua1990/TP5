...
class User extends Controller
{
     ...
     * @throws \Exception
     */
    public function change_referee()
    {   
        $id = $this->request->post("id");
        $model = RefereeModel::detail($id);
        if (!$model->change_referee($this->request->post())) {
            return $this->renderError($model->getError() ?: '更换失败');
        }
        return $this->renderSuccess($model->getError() ?: '更换成功', url('apps.dealer.user/index'));
    }
}