...
use app\store\model\OrderExpress as OrderExpressModel;
...
class Order extends OrderModel
{
    ...
    public function delivery($data)
    {
        ...
        // 更新订单发货状态
        if ($status = $this->updateToDelivery($updateList, $data)) {
            // 保存额外物流订单
            $this->setExtraExpress($data, $this['wxapp_id']);
            // 获取已发货的订单
     ...
     * @throws \think\exception\DbException
     */
    private function setExtraExpress($data, $wxapp_id)
    {   
        // 多个物流订单执行
        if($data['express_single'] == 0){
            $wxapp_id = $this->wxapp_id;
            $order_id = $this->order_id;
            $add = [];
            foreach ($data['express'] as $key => $val) {
                $val['wxapp_id'] = $wxapp_id;
     ...
     * @throws \Exception
     */
    private function updateToDelivery($orderList, $post_data = [])
    {
        $data = [];
        foreach ($orderList as $item) {
            $add = [
            ...
            if($post_data && $post_data['express_single'] == 0){
                $add['express_single'] = $post_data['express_single'];
                $add['express_goods'] = $post_data['express_goods'];
            }
